...
export default {
  ...
  methods: {
    ...
    handlePageQuery: function(pagerRequest) {
      ...
      getDictTypePageInfo(pagerRequest).then(resp => {
    ...
    doUpdate: function() {
      updateDictType(this.updateDialog.formData).then(resp => {
        if (resp.status === 0) {
          this.$message({
            message: '更新成功',
            type: 'success'
    ...
    doAdd: function() {
      addDictType(this.addDialog.formData).then(resp => {
        if (resp.status === 0) {
          this.$message({
            message: '添加成功',
            type: 'success'
    ...
    doDelete: function(data) {
      this.$confirm('你确定要删除 ' + data.typeName + ' 字典类型吗？', '警告', {
